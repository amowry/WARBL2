...
var currentVersion = 47; // Current version of the WARBL2 firmware
...
var fingeringWrite; //indicates the preset for which a fingering pattern is being sent.
...
var instrument = 0; //currently selected preset tab
...
var defaultInstr = 0; //default preset
...
// Add Config Tool version number
let titleElement = document.getElementById("configVersion");
titleElement.append("v." + configVersion);
...
// Import button
document.getElementById("importPresetBtn").onclick = function () {
	document.getElementById("importPresetFile").click();
};

document.getElementById("importPresetFile").onchange = function () {
	importPreset(this);
};
...
function WARBL_Receive(event, source) {
	...
	// (because for some reason the event.target doesn't propogate on the iOS webkit side)
	//debugger;
	...
	if (pressureMappingOpen) { // Only send if the panel is open
		if (mapSelection == 0) {
			...
			if (e == "CC" && (parseFloat(data0 & 0x0f) + 1 == IMUchan2) && data1 == IMUCC2) {
				document.getElementById('receivedpressureCC').innerHTML = data2;
				var barWidth2 = data2 / 1.27;
				document.getElementById("CCpressurelevel").style.width = barWidth2 + "%";
			}
		}
		...
		if (mapSelection == 1) {
			if (e == "On") {
				document.getElementById('receivedpressureCC').innerHTML = data2;
				var barWidth3 = data2 / 1.27;
				document.getElementById("CCpressurelevel").style.width = barWidth3 + "%";
			}
		}
		...
		if (mapSelection == 2) {
			if (e == "CP") {
				...
				var barWidth4 = data1 / 1.27;
				document.getElementById("CCpressurelevel").style.width = barWidth4 + "%";
			}
		}
	...
	// Only send to the synth if it has actually been started
	var synthActive = synthReady && synth;
	...
	switch (data0 & 0xf0) {
		...
		case 0xD0: // Channel Pressure
			if (synthActive) {
				handleChannelPressure(ch, d1);
				handleVolumeControl(ch, d1);
			}
			return;
		...
		case 0xB0: // CC
			...
			if (parseFloat(data0 & 0x0f) == MIDI_CONFIG_TOOL_CHANNEL - 1) { // if it's channel 7 it's from WARBL
				...
				if (data1 == MIDI_CC_105) {
					...
					else if (jumpFactorWrite == 326) {
						patch = data2;
						console.log("Patch = " + patch);
						const el = document.getElementById('version');
						el.appendChild(document.createTextNode('.' + patch));
					...
					if (version < 4.6) {
						...
						for (i = 0; i < 8; ++i) {
							document.getElementById("row" + i).options[17].disabled = true; // Disable "toggle slide" button option.
						}

						document.getElementById("useBellLabel").style.display = "none"; // Hide "use bell sensor" switch.
						document.getElementById("switch35").style.display = "none";
						document.getElementById("thumbHalfHoleControls").style.display = "none"; // Revert to older half hole controls
						document.getElementById("halfHoleControls").style.left = "50px";
						document.getElementById("halfHoleControls").style.top = "-90px";
						document.getElementById("fingers").style.display = "none";
						document.getElementById("fingeringSelect0").options[24].disabled = true; // Disable recorder2 fingering.
					...
					if (version < 3.9) {
						...
						console.log("Original WARBL detected.");
						...
						document.getElementById("fingeringSelect0").options[28].disabled = true;
						document.getElementById("fingeringSelect1").options[24].disabled = true;
						document.getElementById("fingeringSelect1").options[25].disabled = true;
						...
						document.getElementById("fingeringSelect1").options[28].disabled = true;
						document.getElementById("fingeringSelect2").options[24].disabled = true;
						document.getElementById("fingeringSelect2").options[25].disabled = true;
						...
						document.getElementById("fingeringSelect2").options[28].disabled = true;
				...
				if (data1 == MIDI_CC_119) {
					...
					if (WARBL2SettingsReceive == 0) {
					...
					else if (WARBL2SettingsReceive == 3) {
						document.getElementById("checkbox35").checked = data2;
					}
						...
						var x = parseFloat((data2 << 7) | connIntvlLSB);
						x = x / 100;              // convert back to decimal
						x = parseFloat(x.toFixed(1));   // keep 1 digit after decimal					
...
function sendFingeringSelect(row, selection) {
	...
	else { key = 0; } // For WARBL2 always revert key (transpose) to 0 when a new fingering chart is selected
...
function defaultInstrument() { //tell WARBL to set the current preset as the default
...
function handleDefault() { //display correct default preset and "set default" buttons	
...
function sendFingeringRadio(tab) { //change presets, showing the correct tab for each preset.
	...
	backIMU();
	backPressure();
	handleDefault(); //display correct default preset and "set default" buttons	
	customFingeringOkay();
	if (tab == 0) {
		document.getElementById("instrument0").style.display = "block";
		document.getElementById("instrument1").style.display = "none";
...
//Thumb
function sendThumbHalfholeOffset(selection) {
	blink(1);
	selection = parseFloat(selection);
...
function sendThumbHalfholeSize(selection) {
	blink(1);
	selection = parseFloat(selection);
	sendToWARBL(MIDI_CC_104, MIDI_ED_VARS2_START + 40);
	sendToWARBL(MIDI_CC_105, selection);
}


function sendThumbHalfholeRate(selection) {
	blink(1);
	selection = parseFloat(selection);
	sendToWARBL(MIDI_CC_104, MIDI_ED_VARS2_START + 41);
	sendToWARBL(MIDI_CC_105, selection);
}
...
function sendSynthBendRange(selection) {
	blink(1);
	var x = parseFloat(selection);
	if (x < 1 || x > 96 || isNaN(x)) {
		alert("Value must be 1-96.");
		document.getElementById("synthBendRange").value = null;
	} else if (synthReady && synth) {
		setPitchBendRange(x);
		localStorage.setItem("pitchBendRange", x);
	}
}
...
function sendUseBell(selection) {
	blink(1);
	selection = +selection; //convert true/false to 1/0
	sendToWARBL(MIDI_CC_106, MIDI_WARBL2_SETTINGS_START + 3);
	sendToWARBL(MIDI_CC_119, selection);
}
...
function LED2off() {
	elements = document.getElementsByClassName("leddot2");
	for (var i = 0; i < elements.length; i++) {
		elements[i].style.visibility = "hidden";
	}
}
...
var jumpSlider22 = document.getElementById('thumbHalfholeOffset');
...
var jumpSlider23 = document.getElementById('thumbHalfholeSize');
jumpSlider23.addEventListener('input', slider23Change);
function slider23Change() {
	output23.innerHTML = jumpSlider23.value;
}

var output24 = document.getElementById("demo24");
var jumpSlider24 = document.getElementById('thumbHalfholeRate');
jumpSlider24.addEventListener('input', slider24Change);
...
function updateCells() {
	...
	for (var i = 0; i < numberOfGestures; i++) {
		...
		if (((y == 0 || (version > 4.1 && y == 1)) && i < 3 && x == 1) || ((version > 1.4 || version == "Unknown") && i < 3 && (z == 5 || z == 6 || z == 10 || z == 11 || z == 16 || z == 17))) {
			document.getElementById("checkbox" + i).disabled = false;
			document.getElementById("switch" + i).style.cursor = "pointer";
		}

		if (((((y != 0 && version <= 4.1) || (version > 4.1 && y > 1)) && i < 3) || (i < 3 && (x == 0 || x > 1))) && !((version > 1.4 || version == "Unknown") && i < 3 && (z == 5 || z == 6 || z == 10 || z == 11 || z == 16 || z == 17))) {
...
function importPreset(context) {
	...
	var selectedFile = context.files[0];
	...
	// Hide the OK button until after the import is complete
	...
	reader.onload = async function () {
		...
		// Parse the file
		try {
			theImportObject = JSON.parse(reader.result);
		} catch (e) {
			document.getElementById("modal14-title").innerHTML =
				"Error: File is not a WARBL Preset";
			document.getElementById("modal14-ok").style.display = "inline-block";
			return;
		}
		...
		if (theImportObject.version > maxSupportedPresetVersion) {
			document.getElementById("modal14-title").innerHTML =
				"Error: WARBL Preset Version " + theImportObject.version +
				" not supported by this version of the configuration tool";
			document.getElementById("modal14-ok").style.display = "inline-block";
			return;
		}

		if (!theImportObject.messages || theImportObject.messages.length < 12) {
			document.getElementById("modal14-title").innerHTML =
				"Error: Preset file is incomplete";
			document.getElementById("modal14-ok").style.display = "inline-block";
			return;
		}

		if ((theImportObject.messages[0][2] > 40 && version < 4.0) ||
			(theImportObject.messages[0][2] < 40 && version > 4.0)) {
			document.getElementById("modal14-title").innerHTML =
				"Sorry, it is not possible to import settings from the original WARBL to WARBL2 or vice versa.";
			document.getElementById("modal14-ok").style.display = "inline-block";
			return;
		}
		...
		switch (exportedInstrument) {
			...
			default:
				document.getElementById("modal14-title").innerHTML =
					"Error: Invalid instrument data in preset";
				document.getElementById("modal14-ok").style.display = "inline-block";
				return;
		}

		// Skip command 10 - Selected preset at save time
		...
		for (i = msgbaseindex + 12; i < nMessages; ++i) {
			...
			if (byte1 === 106) {
				if (byte2 >= 55 && byte2 <= 57) {
					// Ignore WARBL2 settings data that is independent of preset
					WARBL2SettingsReceive = null;
				} else {
					WARBL2SettingsReceive = 0;
				}
			}

			// Send to WARBL unless we have a WARBL2 setting that shouldn't be changed
			if (!(byte1 === MIDI_CC_119 || (byte1 === MIDI_CC_106 && byte2 >= 55 && byte2 <= 99))) {
				sendToWARBL(byte1, byte2);
				await sleep(delay);
			}
		}
		...
		setTimeout(function () {
			...
			// Show the OK button after the import is complete
			document.getElementById("modal14-ok").style.display = "inline-block";
...
function exportPreset() {
	...
	if (!WARBLout) {
		document.getElementById("modal14-title").innerHTML =
			"Preset export only allowed when WARBL is connected";
	...
	document.getElementById("modal14-ok").style.display = "none";
	document.getElementById("modal14-title").innerHTML = "Preparing export...";
	...
	function downloadPreset(content, filename, contentType) {
		...
		// Delay revoke for Safari safety
		setTimeout(function () {
	...
	//
	// Finalize export after messages have been collected
	//
	function exportFunction() {
		...
		if (platform == "app" || platform == "app2") {
			...
			document.getElementById("modal14-title").innerHTML =
				"Preset ready. Tap Save and select Save to Files.";
			...
			// Original behavior for non-app platforms
...
//
// Called by the Save button in the modal
//
async function saveExportedPreset() {

	if (!gPendingPresetExport) {
		document.getElementById("modal14-title").innerHTML =
			"No preset is ready to save.";
		document.getElementById("modal14-ok").style.display = "inline-block";
		modal(14);
		return;
	}
	...
	try {
		const file = new File(
			[gPendingPresetExport.content],
			gPendingPresetExport.filename,
			{ type: gPendingPresetExport.contentType }
		);
		...
		if (navigator.canShare && navigator.canShare({ files: [file] })) {
			...
			a.href = url;
			a.download = gPendingPresetExport.filename;
		...
		gPendingPresetExport = null;

		if (saveBtn) saveBtn.style.display = "none";

		// Only show OK after successful save
		document.getElementById("modal14-ok").style.display = "inline-block";
		modal(14);
	}
	catch (err) {
		...
		try {
			...
			if ("caches" in window) {
				try {
					...
					console.log("All cache storage entries deleted");
				}
				catch (error) {
