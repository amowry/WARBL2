...
WARBLin=source;if(WARBLout&&WARBLin){console.log("Connected WARBL!");console.log(WARBLin);enableMidiInputForOnly(WARBLin.id);updateMidiDeviceSelector()}}}
...
if(mapSelection==0){var IMUchan2=document.getElementById('pressureChannel').value;var IMUCC2=document.getElementById('pressureCC').value;if(e=="CC"&&(parseFloat(data0&0x0f)+1==IMUchan2)&&data1==IMUCC2){document.getElementById('receivedpressureCC').innerHTML=data2;var barWidth2=data2/1.27;document.getElementById("CCpressurelevel").style.width=barWidth2+"%"}}
if(mapSelection==1){if(e=="On"){document.getElementById('receivedpressureCC').innerHTML=data2;var barWidth3=data2/1.27;document.getElementById("CCpressurelevel").style.width=barWidth3+"%"}}
if(mapSelection==2){if(e=="CP"){document.getElementById('receivedpressureCC').innerHTML=data1;var barWidth4=data1/1.27;document.getElementById("CCpressurelevel").style.width=barWidth4+"%"}}
if(mapSelection==3){if(e=="KP"){document.getElementById('receivedpressureCC').innerHTML=data2;var barWidth5=data2/1.27;document.getElementById("CCpressurelevel").style.width=barWidth5+"%"}}
...
var ch=data0&0x0F;var d1=data1||0;var d2=data2||0;var synthActive=synthReady&&synth;switch(data0&0xf0){case 0x90:if(d2!==0){notePlaying=d1;logKeys;document.getElementById("tinyConsole").value=noteNames[d1-1]+" "+d1;if(synthActive){synthNoteOn(ch,d1,d2);LED2on()}}else{if(synthActive){synthNoteOff(ch,d1)}
logKeys;if(notePlaying==d1){document.getElementById("tinyConsole").value="";notePlaying=-1;LED2off()}}
return;case 0x80:if(synthActive){synthNoteOff(ch,d1)}
logKeys;if(notePlaying==d1){document.getElementById("tinyConsole").value="";notePlaying=-1;LED2off()}
return;case 0xE0:if(synthActive){handlePitchBend(ch,(d2<<7)|d1)}
return;case 0xD0:if(synthActive){handleChannelPressure(ch,d1);handleVolumeControl(ch,d1)}
return;case 0xB0:if(synthActive){handleControlChange(ch,d1,d2)}
if(MIDI_DEBUG){console.log("From WARBL",data1,data2)}
...
if(version<4.6){for(i=0;i<8;++i){document.getElementById("row"+i).options[17].disabled=!0}
...
if(version<3.9){console.log("Original WARBL detected.");numberOfGestures=8;for(let element of document.getElementsByClassName("WARBL2Elements")){element.style.display="none"}
...
if(data1==MIDI_CC_119){if(WARBL2SettingsReceive==0){document.getElementById("WARBL2Radio"+data2).checked=!0}else if(WARBL2SettingsReceive==1){document.getElementById("checkbox19").checked=data2}else if(WARBL2SettingsReceive==3){document.getElementById("checkbox35").checked=data2}else if(WARBL2SettingsReceive==2){document.getElementById("poweroffSlider").value=data2;var value=data2;if(version>4.4&&value==25){value="∞"}
...
function handleDefault(){if(version>1.6||version=="Unknown"){if(defaultInstr!=0){document.getElementById("instrument1Label").innerHTML="Preset 1:"}
if(defaultInstr!=1){document.getElementById("instrument2Label").innerHTML="Preset 2:"}
if(defaultInstr!=2){document.getElementById("instrument3Label").innerHTML="Preset 3:"}
if(defaultInstr==0){document.getElementById("instrument1Label").innerHTML="Preset 1 (default):"}
if(defaultInstr==1){document.getElementById("instrument2Label").innerHTML="Preset 2 (default):"}
if(defaultInstr==2){document.getElementById("instrument3Label").innerHTML="Preset 3 (default):"}
...
function sendThumbHalfholeOffset(selection){blink(1);selection=parseFloat(selection);sendToWARBL(MIDI_CC_104,MIDI_ED_VARS2_START+39);sendToWARBL(MIDI_CC_105,selection)}
function sendThumbHalfholeSize(selection){blink(1);selection=parseFloat(selection);sendToWARBL(MIDI_CC_104,MIDI_ED_VARS2_START+40);sendToWARBL(MIDI_CC_105,selection)}
function sendThumbHalfholeRate(selection){blink(1);selection=parseFloat(selection);sendToWARBL(MIDI_CC_104,MIDI_ED_VARS2_START+41);sendToWARBL(MIDI_CC_105,selection)}
...
function sendSynthBendRange(selection){blink(1);var x=parseFloat(selection);if(x<1||x>96||isNaN(x)){alert("Value must be 1-96.");document.getElementById("synthBendRange").value=null}else if(synthReady&&synth){setPitchBendRange(x);localStorage.setItem("pitchBendRange",x)}}
...
function sendUseBell(selection){blink(1);selection=+selection;sendToWARBL(MIDI_CC_106,MIDI_WARBL2_SETTINGS_START+3);sendToWARBL(MIDI_CC_119,selection)}
...
if(modalId==32){if(volume==0){toggleOn()}}}
...
function LED2off(){elements=document.getElementsByClassName("leddot2");for(var i=0;i<elements.length;i++){elements[i].style.visibility="hidden"}}
function LED2on(){elements=document.getElementsByClassName("leddot2");for(var i=0;i<elements.length;i++){elements[i].style.visibility="visible"}}
...
var output22=document.getElementById("demo22");var jumpSlider22=document.getElementById('thumbHalfholeOffset');jumpSlider22.addEventListener('input',slider22Change);function slider22Change(){output22.innerHTML=jumpSlider22.value}
var output23=document.getElementById("demo23");var jumpSlider23=document.getElementById('thumbHalfholeSize');jumpSlider23.addEventListener('input',slider23Change);function slider23Change(){output23.innerHTML=jumpSlider23.value}
var output24=document.getElementById("demo24");var jumpSlider24=document.getElementById('thumbHalfholeRate');jumpSlider24.addEventListener('input',slider24Change);function slider24Change(){output24.innerHTML=jumpSlider24.value}
...
var z=document.getElementById("row"+i).value;if(((y==0||(version>4.1&&y==1))&&i<3&&x==1)||((version>1.4||version=="Unknown")&&i<3&&(z==5||z==6||z==10||z==11||z==16||z==17))){document.getElementById("checkbox"+i).disabled=!1;document.getElementById("switch"+i).style.cursor="pointer"}
if(((((y!=0&&version<=4.1)||(version>4.1&&y>1))&&i<3)||(i<3&&(x==0||x>1)))&&!((version>1.4||version=="Unknown")&&i<3&&(z==5||z==6||z==10||z==11||z==16||z==17))){document.getElementById("checkbox"+i).disabled=!0;document.getElementById("switch"+i).style.cursor="default";document.getElementById("checkbox"+i).checked=!1}
var q=document.getElementById("checkbox0").checked;if(q==!0){document.getElementById("row5").disabled=!0}else{document.getElementById("row5").disabled=!1}
var r=document.getElementById("checkbox1").checked;if(r==!0){document.getElementById("row6").disabled=!0}else{document.getElementById("row6").disabled=!1}
if(q==!0||r==!0){document.getElementById("row3").disabled=!0}else{document.getElementById("row3").disabled=!1}
var s=document.getElementById("checkbox2").checked;if(s==!0){document.getElementById("row7").disabled=!0}else{document.getElementById("row7").disabled=!1}
...
function topNavFunction(){var x=document.getElementById("myTopnav");if(x.className==="topnav"){x.className+=" responsive"}else{x.className="topnav"}}
function sleep(ms){return new Promise(resolve=>setTimeout(resolve,ms))}
function importPreset(context){console.log("importPreset");console.log("current tab = "+instrument);if(!WARBLout){document.getElementById("modal14-title").innerHTML="Preset import only allowed when WARBL is connected";document.getElementById("modal14-ok").style.display="inline-block";modal(14);return}
var maxSupportedPresetVersion=1;var delay=10;if(!(context.files&&context.files.length!==0)){return}
var selectedFile=context.files[0];var fname=selectedFile.name;fname=fname.replace(".warbl","");fname=fname.replace(".WARBL","");document.getElementById("modal14-title").innerHTML="Importing: "+selectedFile.name;document.getElementById("modal14-ok").style.display="none";modal(14);var reader=new FileReader();reader.onload=async function(){var theImportObject;try{theImportObject=JSON.parse(reader.result)}catch(e){document.getElementById("modal14-title").innerHTML="Error: File is not a WARBL Preset";document.getElementById("modal14-ok").style.display="inline-block";return}
if(!theImportObject){document.getElementById("modal14-title").innerHTML="Error: File is not a WARBL Preset";document.getElementById("modal14-ok").style.display="inline-block";return}
if(!theImportObject.signature||theImportObject.signature!=="WARBL"){document.getElementById("modal14-title").innerHTML="Error: File is not a WARBL Preset";document.getElementById("modal14-ok").style.display="inline-block";return}
if(!theImportObject.version){document.getElementById("modal14-title").innerHTML="Error: File is not a WARBL Preset";document.getElementById("modal14-ok").style.display="inline-block";return}
if(theImportObject.version>maxSupportedPresetVersion){document.getElementById("modal14-title").innerHTML="Error: WARBL Preset Version "+theImportObject.version+" not supported by this version of the configuration tool";document.getElementById("modal14-ok").style.display="inline-block";return}
if(!theImportObject.messages||theImportObject.messages.length<12){document.getElementById("modal14-title").innerHTML="Error: Preset file is incomplete";document.getElementById("modal14-ok").style.display="inline-block";return}
if((theImportObject.messages[0][2]>40&&version<4.0)||(theImportObject.messages[0][2]<40&&version>4.0)){document.getElementById("modal14-title").innerHTML="Sorry, it is not possible to import settings from the original WARBL to WARBL2 or vice versa.";document.getElementById("modal14-ok").style.display="inline-block";return}
var nMessages=theImportObject.messages.length;var i;var byte1,byte2;byte1=theImportObject.messages[0][1];byte2=theImportObject.messages[0][2];sendToWARBL(byte1,byte2);await sleep(delay);var exportedInstrument=theImportObject.messages[10][2]-60;var targetInstrument=30+instrument;console.log("exportedInstrument = "+exportedInstrument);console.log("targetInstrument = "+(targetInstrument-30));var targetKey=111+instrument;switch(exportedInstrument){case 0:byte1=theImportObject.messages[1][2];byte2=targetInstrument;sendToWARBL(byte1,byte2);await sleep(delay);byte1=theImportObject.messages[2][1];byte2=theImportObject.messages[2][2];sendToWARBL(byte1,byte2);await sleep(delay);byte1=targetKey;byte2=theImportObject.messages[3][2];sendToWARBL(byte1,byte2);await sleep(delay);break;case 1:byte1=theImportObject.messages[4][1];byte2=targetInstrument;sendToWARBL(byte1,byte2);await sleep(delay);byte1=theImportObject.messages[5][1];byte2=theImportObject.messages[5][2];sendToWARBL(byte1,byte2);await sleep(delay);byte1=targetKey;byte2=theImportObject.messages[6][2];sendToWARBL(byte1,byte2);await sleep(delay);break;case 2:byte1=theImportObject.messages[7][1];byte2=targetInstrument;sendToWARBL(byte1,byte2);await sleep(delay);byte1=theImportObject.messages[8][1];byte2=theImportObject.messages[8][2];sendToWARBL(byte1,byte2);await sleep(delay);byte1=targetKey;byte2=theImportObject.messages[9][2];sendToWARBL(byte1,byte2);await sleep(delay);break;default:document.getElementById("modal14-title").innerHTML="Error: Invalid instrument data in preset";document.getElementById("modal14-ok").style.display="inline-block";return}
for(i=12;i<nMessages;++i){byte1=theImportObject.messages[i][1];byte2=theImportObject.messages[i][2];if(byte1===106){if(byte2>=55&&byte2<=57){WARBL2SettingsReceive=null}else{WARBL2SettingsReceive=0}}
if(!(byte1===MIDI_CC_119||(byte1===MIDI_CC_106&&byte2>=55&&byte2<=99))){sendToWARBL(byte1,byte2);await sleep(delay)}}
setTimeout(function(){sendToWARBL(MIDI_CC_102,MIDI_ENTER_COMM_MODE);document.getElementById("modal14-title").innerHTML="Preset Import Complete!";document.getElementById("modal14-ok").style.display="inline-block";setTimeout(function(){document.getElementById("importPresetFile").value=""},1000)},500)};reader.readAsText(selectedFile)}
let gPendingPresetExport=null;function exportPreset(){if(!WARBLout){document.getElementById("modal14-title").innerHTML="Preset export only allowed when WARBL is connected";document.getElementById("modal14-ok").style.display="inline-block";const saveBtn=document.getElementById("modal14-save");if(saveBtn)saveBtn.style.display="none";modal(14);return}
var nMessages=0;document.getElementById("modal14-ok").style.display="none";document.getElementById("modal14-title").innerHTML="Preparing export...";const saveBtn=document.getElementById("modal14-save");if(saveBtn)saveBtn.style.display="none";modal(14);var theExportObject={signature:"WARBL",version:1,messages:[]};function downloadPreset(content,filename,contentType){const a=document.createElement('a');const file=new Blob([content],{type:contentType});const url=URL.createObjectURL(file);a.href=url;a.download=filename;document.body.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(url)},10000);document.getElementById("modal14-title").innerHTML="Preset Export Complete!";document.getElementById("modal14-ok").style.display="inline-block";if(saveBtn)saveBtn.style.display="none";modal(14)}
function exportMessageHandler(byte0,byte1,byte2){theExportObject.messages.push([byte0,byte1,byte2]);nMessages++}
function exportFunction(){var theExportObjectJSON=JSON.stringify(theExportObject);gExportProxy=null;if(platform=="app"||platform=="app2"){gPendingPresetExport={content:theExportObjectJSON,filename:"WARBL_Preset.warbl",contentType:"application/octet-stream"};document.getElementById("modal14-ok").style.display="none";document.getElementById("modal14-title").innerHTML="Preset ready. Tap Save and select Save to Files.";if(saveBtn){saveBtn.style.display="inline-block"}
modal(14)}else{downloadPreset(theExportObjectJSON,"WARBL_Preset.warbl","text/plain")}}
gExportProxy=exportMessageHandler;sendToWARBL(MIDI_CC_102,MIDI_ENTER_COMM_MODE);setTimeout(exportFunction,3000)}
async function saveExportedPreset(){if(!gPendingPresetExport){document.getElementById("modal14-title").innerHTML="No preset is ready to save.";document.getElementById("modal14-ok").style.display="inline-block";modal(14);return}
const saveBtn=document.getElementById("modal14-save");try{const file=new File([gPendingPresetExport.content],gPendingPresetExport.filename,{type:gPendingPresetExport.contentType});if(navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({title:"WARBL Preset",files:[file]});document.getElementById("modal14-title").innerHTML="Preset Export Complete!"}else{const url=URL.createObjectURL(file);const a=document.createElement("a");a.href=url;a.download=gPendingPresetExport.filename;document.body.appendChild(a);a.click();a.remove();setTimeout(function(){URL.revokeObjectURL(url)},10000);document.getElementById("modal14-title").innerHTML="Preset Export Complete!"}
gPendingPresetExport=null;if(saveBtn)saveBtn.style.display="none";document.getElementById("modal14-ok").style.display="inline-block";modal(14)}catch(err){document.getElementById("modal14-title").innerHTML="Save canceled or not supported on this device.";document.getElementById("modal14-ok").style.display="none";if(saveBtn)saveBtn.style.display="inline-block";modal(14)}}
...
function sendUpdateTool(){modalclose(33);console.log("Tool update requested");setTimeout(async function(){try{if("serviceWorker" in navigator){try{const registrations=await navigator.serviceWorker.getRegistrations();for(const registration of registrations){console.log("Unregistering service worker with scope: "+registration.scope);await registration.unregister()}
console.log("All service workers unregistered")}catch(error){console.error("Error unregistering service workers:",error)}}else{console.warn("Service workers are not supported in this browser.")}
if("caches" in window){try{const cacheNames=await caches.keys();for(const cacheName of cacheNames){console.log("Deleting cache: "+cacheName);await caches.delete(cacheName)}
console.log("All cache storage entries deleted")}catch(error){console.error("Error deleting cache storage:",error)}}else{console.warn("Cache Storage API is not supported in this browser.")}}finally{finishUpdateTool()}},100)}
function finishUpdateTool(){setTimeout(function(){modal(34)},500)}
function restartTool(){window.location.reload()}