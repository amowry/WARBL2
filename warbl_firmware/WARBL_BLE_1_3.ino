...
const uint8_t chargeEnable = 7;  //Enables charging with a current ~160-180 mA (0.4-0.45C, should take around 2.5 hours to charge)

const uint8_t battTempEnable = 6;  //Driving this high enables the thermistor for measuring battery temp. We normally keep the thermistor off so it doesn't self-heat and also use current.
const uint8_t battTempRead = 15;   //Thermistor ********This will become STAT pin  -- input with pullup  ***********************************

const uint8_t battReadEnable = 11;  //Driving this high connects the battery to NRF for reading voltage
...
void loop() {
    ...
    /////////// Things here happen ~ every 20 mS
